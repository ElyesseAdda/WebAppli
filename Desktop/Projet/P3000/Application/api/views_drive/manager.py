...
class DriveManager:
    ...
    def create_empty_document(
        ...
        """
        Crée un fichier Office vide (Word, Excel, PowerPoint)
        ...
        try:
            ...
            # Déterminer l'extension et le contenu selon le type
            if document_type == 'word':
                extension = '.docx'
                file_content = self._create_empty_docx()
            elif document_type == 'excel':
                extension = '.xlsx'
                file_content = self._create_empty_xlsx()
            elif document_type == 'powerpoint':
                extension = '.pptx'
                file_content = self._create_empty_pptx()
            else:
                raise ValueError(f"Type de document non supporté: {document_type}")
            
            # Construire le chemin complet
            full_key = f"{folder_path}{file_name_normalized}{extension}"
            
            # Upload le fichier sur S3
            success = self.storage.upload_file_content(
                key=full_key,
            ...
            if success:
                return {
                    'success': True,
                    'file_path': full_key,
                    'file_name': f"{file_name_normalized}{extension}",
                    'file_name_display': f"{file_name}{extension}"
                }
            else:
                raise Exception("Échec de la création du fichier")
            
        except Exception as e:
            raise
    
    def _create_empty_docx(self) -> bytes:
        """
        Crée un fichier Word (.docx) vide
...
<w:qFormat/>
    ...
    def _create_empty_xlsx(self) -> bytes:
        """
        Crée un fichier Excel (.xlsx) vide
        """
        zip_buffer = io.BytesIO()
...
<xf/>
    ...
    def _create_empty_pptx(self) -> bytes:
        """
        Crée un fichier PowerPoint (.pptx) vide
        """
        zip_buffer = io.BytesIO()
...
<grpSpPr/>
            ...
            zip_file.writestr('ppt/slides/slide1.xml', slide)
...
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>
            ...
            zip_file.writestr('ppt/slides/_rels/slide1.xml.rels', slide_rels)
            
            # ppt/slideLayouts/slideLayout1.xml (simplifié)
...
<grpSpPr/>
            ...
            zip_file.writestr('ppt/slideLayouts/slideLayout1.xml', layout)
