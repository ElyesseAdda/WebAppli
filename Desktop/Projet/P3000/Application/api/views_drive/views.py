...
class DriveV2ViewSet(viewsets.ViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='file-diagnostics')
    def file_diagnostics(self, request):
        """
        Endpoint de diagnostic pour vérifier l'état d'un fichier
        
        Query params:
            - file_path: Chemin du fichier à diagnostiquer
        """
        try:
            file_path = request.query_params.get('file_path')
            ...
            metadata = self.drive_manager.storage.get_object_metadata(file_path)
            
            if not metadata:
                return Response({
                    'exists': False,
                    'file_path': file_path,
                    'error': 'Fichier non trouvé sur S3'
                }, status=status.HTTP_404_NOT_FOUND)
            
            # Calculer un hash du contenu pour vérifier l'intégrité
            import hashlib
            try:
                file_content = self.drive_manager.storage.download_file_content(file_path)
            ...
            return Response({
                ...
                'content': {
                    ...
                    'error': content_error if 'content_error' in locals() else None
                },
                'download_url_info': {
                    'url_generated': True,
            ...
            logger.error(f"Erreur dans file_diagnostics: {str(e)}")
            logger.error(traceback.format_exc())
            return Response(
                {'error': str(e), 'traceback': traceback.format_exc() if settings.DEBUG else None},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
