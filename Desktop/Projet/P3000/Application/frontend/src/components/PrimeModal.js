...
const PrimeModal = ({ isOpen, onClose, month, year }) => {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      await Promise.all([loadAgents(), loadChantiers(), loadAgences(), loadExistingPrimes()]);
    } catch (err) {
  ...
  const handleAddPrime = async (agentId) => {
    ...
    if (primeData.type_affectation === "chantier" && !primeData.chantier) {
      alert("Veuillez sélectionner un chantier");
      return;
    }

    if (primeData.type_affectation === "agence" && !primeData.agence) {
    ...
    try {
      await axios.post("/api/agent-primes/", {
        ...
        chantier: primeData.type_affectation === "chantier" ? primeData.chantier : null,
        agence: primeData.type_affectation === "agence" ? primeData.agence : null,
      });
  ...
  const getAffectationLabel = (prime) => {
    if (prime.type_affectation === "agence") {
      return prime.agence_nom || "Agence";
    }
    return prime.chantier_name || "Chantier";
  };
                      ...
                      <AccordionDetails sx={{ p: 3 }}>
                        ...
                        >
                          <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 3 }}>
                            ...
                            <Typography variant="h6" sx={{ fontWeight: 600, color: "rgba(27, 120, 188, 1)" }}>
                          ...
                          <Grid container spacing={2}>
                            <Grid item xs={12} md={6}>
                              <TextField
                                ...
                                onChange={(e) => handleNewPrimeChange(agent.id, "montant", e.target.value)}
                                InputProps={{
                                  startAdornment: (
                            ...
                            <Grid item xs={12} md={6}>
                              <TextField
                                ...
                                label="Affectation"
                                fullWidth
                                value={newPrimes[agent.id]?.type_affectation || "agence"}
                                onChange={(e) => handleNewPrimeChange(agent.id, "type_affectation", e.target.value)}
                                InputProps={{
                                  startAdornment: (
                                    <InputAdornment position="start">
                            ...
                            <Grid item xs={12}>
                              <TextField
                                ...
                                onChange={(e) => handleNewPrimeChange(agent.id, "description", e.target.value)}
                            ...
                            {newPrimes[agent.id]?.type_affectation === "agence" && (
                              <Grid item xs={12}>
                                <TextField
                                  ...
                                  label="Agence"
                                  fullWidth
                                  value={newPrimes[agent.id]?.agence || ""}
                                  onChange={(e) => handleNewPrimeChange(agent.id, "agence", e.target.value)}
                                  InputProps={{
                                    startAdornment: (
                                      <InputAdornment position="start">
                                        <BusinessIcon sx={{ color: "rgba(27, 120, 188, 1)" }} />
                            ...
                            {newPrimes[agent.id]?.type_affectation === "chantier" && (
                              <Grid item xs={12}>
                                <TextField
                                  select
                                  label="Chantier"
                                  ...
                                  onChange={(e) => handleNewPrimeChange(agent.id, "chantier", e.target.value)}
                                  InputProps={{
                                    startAdornment: (
                                      <InputAdornment position="start">
                                        <ConstructionIcon sx={{ color: "rgba(27, 120, 188, 1)" }} />
                            ...
                            <Grid item xs={12}>
                              <Button
                                ...
                                disabled={
                                  ...
                                  (newPrimes[agent.id]?.type_affectation === "agence" && !newPrimes[agent.id]?.agence) ||
                                  (newPrimes[agent.id]?.type_affectation === "chantier" && !newPrimes[agent.id]?.chantier)
                                }
                                startIcon={<AddIcon />}
                                sx={{
                                  backgroundColor: "rgba(27, 120, 188, 1)",
                                ...
                                <Grid item xs={12} key={prime.id}>
                                  ...
                                  >
                                    <Box sx={{ flex: 1 }}>
                                      ...
                                      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                                        <Chip
                                          icon={prime.type_affectation === "agence" ? <BusinessIcon /> : <ConstructionIcon />}
                                          label={getAffectationLabel(prime)}
                                          size="small"
                                          sx={{
                                            backgroundColor: prime.type_affectation === "agence"
                                              ? "rgba(255, 152, 0, 0.1)"
                                              : "rgba(76, 175, 80, 0.1)",
                                            color: prime.type_affectation === "agence"
                                              ? "#f57c00"
                                              : "#388e3c",
                                            border: `1px solid ${prime.type_affectation === "agence" ? "rgba(255, 152, 0, 0.3)" : "rgba(76, 175, 80, 0.3)"}`,
                                            fontWeight: 500,
                                          }}
                                        />
                                        <Typography
