...
const StockHistoryCard = () => {
  ...
  const fetchHistory = (page = 1) => {
    axios
      .get(`/api/historique_stock/?page=${page}`)
      .then((response) => {
    ...
    return Object.keys(groupedHistory).reduce((result, date) => {
      ...
      const filteredOperations = operations.filter((item) => {
        const matchType = filterType
          ? item.type_operation === filterType
          : true;

        // Ajouter le signe négatif si c'est un retrait
        const montantString =
          item.type_operation === "retrait"
        ...
        const matchSearch = searchQuery
          ? item.stock?.designation
              ?.toLowerCase()
              .includes(searchQuery.toLowerCase()) ||
            item.stock?.code_produit
              ?.toLowerCase()
              .includes(searchQuery.toLowerCase()) ||
            (item.chantier?.chantier_name &&
              item.chantier.chantier_name
                .toLowerCase()
                .includes(searchQuery.toLowerCase())) ||
            (item.agent?.name &&
              item.agent.name
                .toLowerCase()
                .includes(searchQuery.toLowerCase())) ||
            montantString.includes(searchQuery) ||
            quantiteString.includes(searchQuery) ||
          ...
          {Object.keys(filteredHistory).map((date, index) => {
            ...
            return (
              ...
              >
                ...
                {totalAjout > 0 && (
                  <p style={{ color: "green" }}>
                    Ajouts : {totalAjout.toFixed(2)} €
                  </p>
                )}
                {totalRetrait > 0 && (
                  <p style={{ color: "red" }}>
                    Retraits : -{totalRetrait.toFixed(2)} €
                  </p>
                )}
        ...
        <div style={{ marginTop: "40px" }}>
          ...
          <div style={{ marginTop: "20px", fontWeight: "700" }}>
            {(() => {
              ...
              const { totalAjout, totalRetrait } =
                calculateSums(filteredOperations);
              return (
                <>
                  <p style={{ color: "green" }}>
                    Total Ajouts : {totalAjout.toFixed(2)} €
                  </p>
                  <p style={{ color: "red" }}>
                    Total Retraits : -{totalRetrait.toFixed(2)} €
                  </p>
                </>
              );
            })()}
