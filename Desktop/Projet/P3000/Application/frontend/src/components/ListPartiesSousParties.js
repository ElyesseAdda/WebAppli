...
const ListePartiesSousParties = () => {
  ...
  const handleUpdate = () => {
    if (editMode.type === "partie") {
      axios
        .put(`/api/parties/${editMode.id}/`, { titre: editValue })
        .then(() => {
      ...
      axios
        .put(`/api/sous-parties/${editMode.id}/`, {
          description: editValue,
          partie: editMode.relatedPartieId,
        })
        .then(() => {
        ...
        .catch((error) => {
          console.error(
            "Erreur lors de la mise à jour de la sous-partie",
            error
          );
        });
      ...
      axios
        .put(`/api/ligne-details/${editMode.id}/`, {
          description,
          unite,
        ...
        .catch((error) => {
          console.error(
            "Erreur lors de la mise à jour de la ligne de détail",
            error
          );
        });
    }
  ...
  const handleDelete = () => {
    ...
    if (confirmed) {
      if (editMode.type === "partie") {
        axios
          .delete(`/api/parties/${editMode.id}/`)
          .then(() => {
        ...
        axios
          .delete(`/api/sous-parties/${editMode.id}/`)
          .then(() => {
          ...
          .catch((error) => {
            console.error(
              "Erreur lors de la suppression de la sous-partie",
              error
            );
          });
      } else if (editMode.type === "ligne") {
        axios
          .delete(`/api/ligne-details/${editMode.id}/`)
          .then(() => {
          ...
          .catch((error) => {
            console.error(
              "Erreur lors de la suppression de la ligne de détail",
              error
            );
          });
      }
      ...
      (partie.sous_parties &&
        partie.sous_parties.some(
          (sousPartie) =>
            ...
            (sousPartie.lignes_details &&
              sousPartie.lignes_details.some(
                (ligne) =>
  ...
  return (
    <div className="main-container">
          ...
          {filteredParties.map((partie) => (
            <div key={partie.id}>
              {editMode.type === "partie" && editMode.id === partie.id ? (
                ...
                <h3
                  onClick={() =>
                    handleEditClick("partie", partie.id, partie.titre)
                  }
                >
                  {partie.titre}
                </h3>
              ...
              {partie.sous_parties.map((sousPartie) => (
                <div key={sousPartie.id}>
                  {editMode.type === "sousPartie" &&
                  editMode.id === sousPartie.id ? (
                    <input
                      type="text"
                      value={editValue}
                      onChange={(e) => setEditValue(e.target.value)}
      ...
      {editMode.id && (
        <div>
          <button className="valider" onClick={handleUpdate}>
            Valider
          </button>
          <button className="supprimer" onClick={handleDelete}>
            Supprimer
          </button>
        </div>
      )}
