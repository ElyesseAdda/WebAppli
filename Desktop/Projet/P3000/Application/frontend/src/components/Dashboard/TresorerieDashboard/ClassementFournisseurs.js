...
const ChantierDetailModal = ({ fournisseur, open, onClose }) => {
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ pt: 2 }}>
        ...
        >
          {[
            { label: "Total facturé", value: fournisseur.totalAPayer, color: "#1B78BC" },
            { label: "Payé", value: fournisseur.totalPaye, color: "#16a34a" },
            { label: "Reste à payer", value: fournisseur.resteAPayer, color: "#dc2626" },
        ...
        <TableContainer>
          <Table size="small">
            ...
            <TableBody>
              {[...fournisseur.chantiers]
                .sort((a, b) => b.totalFactures - a.totalFactures)
                .map((ch) => {
                  const pct =
                    ch.totalAPayer > 0 ? (ch.totalPaye / ch.totalAPayer) * 100 : 0;
                  return (
                    <TableRow
                      key={ch.chantier_id}
...
/** Composant principal — classement des fournisseurs */
    ...
    <>
      <TableContainer sx={{ maxHeight: 420, overflowY: "auto" }}>
        <Table size="small" stickyHeader>
          <TableHead>
            <TableRow>
              {["#", "Fournisseur", "Total facturé", "Payé", "Reste à payer", "Avancement", ""].map(
                (h) => (
                  <TableCell
                    key={h}
                    sx={{
            ...
            {classement.map((row, idx) => (
              ...
              >
                ...
                <TableCell sx={{ py: 1, maxWidth: 180 }}>
                  <Tooltip title={row.fournisseur} placement="top">
                    <Typography
                      sx={{
                        fontSize: "0.82rem",
                        fontWeight: 600,
                ...
                <TableCell sx={{ py: 1, pr: 1 }}>
                  <Tooltip title="Détail par chantier">
                    <IconButton
                      size="small"
                      onClick={() => setSelectedFournisseur(row)}
                      sx={{
                        color: "#94a3b8",
      ...
      <Box sx={{ display: "flex", gap: 2, mt: 1.5, flexWrap: "wrap" }}>
        {[
          { color: "#1B78BC", label: "Total facturé" },
