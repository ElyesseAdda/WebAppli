...
| `api/pdf_manager.py` | **MODIFIÉ** : `generate_andStore_pdf()` et `replace_file_with_confirmation()` | Remplacement des noms de fichiers temporaires fixes (`/tmp/facture_temp.pdf`, etc.) par `tempfile.NamedTemporaryFile` (noms uniques). Corrige `EACCES: permission denied` quand un fichier créé par `root` bloque `www-data`. Ajout bloc `finally` pour nettoyage. Inclut `e.stderr` dans les messages d'erreur `CalledProcessError`. |
