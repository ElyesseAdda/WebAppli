...
const StockEntriesHistory = () => {
    ...
    const fetchEntriesHistory = () => {
        axios.get('/api/stock-entries')  // Change cette route si nécessaire
            .then(response => {
                setEntriesHistory(response.data);
            })
            .catch(error => {
                console.error("Erreur lors du chargement de l'historique des entrées:", error);
            });
    };
            ...
            <table style={tableStyle}>
                ...
                <tbody>
                    {entriesHistory.map((entry, index) => (
                        <tr key={entry.id} style={{ backgroundColor: index % 2 === 0 ? '#f9f9f9' : '#ffffff' }}>
                            <td style={cellStyle}>{entry.code_produit}</td>
                            <td style={cellStyle}>{entry.nom_materiel}</td>
                            ...
                            <td style={cellStyle}>{new Date(entry.date_commande).toLocaleDateString()}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
