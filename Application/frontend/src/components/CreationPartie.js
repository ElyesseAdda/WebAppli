...
const CreationPartie = ({ refreshData }) => {
    ...
    const handleAddLigneDetail = () => {
        setLignesDetails([...lignesDetails, { description: '', unite: '', prix: '' }]);
    };
    ...
    const handleCreationSubmit = () => {
        if (creationType === 'partie') {
            ...
            axios.post('/api/parties/', { titre: newPartie })
                .then(response => {
                    ...
                    if (newSousPartie.trim()) {
                        axios.post('/api/sous-parties/', { description: newSousPartie, partie: partieId })
                            .then(response => {
            ...
            axios.post('/api/sous-parties/', { description: newSousPartie, partie: selectedPartieId })
                .then(response => {
                    const sousPartieId = response.data.id;
    ...
    return (
        <div className='creation-partie-container'>
            <button className='add-button' onClick={() => setShowCreationForm(!showCreationForm)}>
                {showCreationForm ? <FaMinusCircle /> : <span><FaPlusCircle /> Nouvelle Partie</span>}
            </button>

            {showCreationForm && (
                <div className='creation-form'>
                    <h2>Création d'une Nouvelle Partie, Sous-Partie ou Ligne de Détail</h2>
                    ...
                    {creationType === 'partie' && (
                        <div className='new-creation-section'>
                            ...
                            <button onClick={handleAddLigneDetail}>Ajouter une Ligne de Détail</button>
                        </div>
                    )}
                    ...
                    <button className='valider' onClick={handleCreationSubmit}>Valider</button>
                    <button className='supprimer' onClick={resetForm}>Reset</button>
                </div>
            )}
        </div>
    );
